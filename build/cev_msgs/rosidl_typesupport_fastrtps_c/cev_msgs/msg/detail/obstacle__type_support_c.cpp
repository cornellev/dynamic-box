...
#include "cev_msgs/msg/detail/obstacle__rosidl_typesupport_fastrtps_c.h"
...
using _Obstacle__ros_msg_type = cev_msgs__msg__Obstacle;
...
static uint32_t _Obstacle__get_serialized_size(const void * untyped_ros_message)
{
  return static_cast<uint32_t>(
    get_serialized_size_cev_msgs__msg__Obstacle(
  ...
  size_t current_alignment)
{
  ...
  if (is_plain) {
    ...
    using DataType = cev_msgs__msg__Obstacle;
    is_plain =
      (
      offsetof(DataType, max_radius) +
      last_member_size
      ) == ret_val;
  }
...
static size_t _Obstacle__max_serialized_size(char & bounds_info)
{
  ...
  ret_val = max_serialized_size_cev_msgs__msg__Obstacle(
